...
SpriteSheet::SpriteSheet(SDL_Renderer *r, SDL_Texture *&&t, int w, int h,
    unsigned int rows, unsigned int cols) noexcept :
    _rdr(r), _sheet(t), _w(w), _h(h), _rows(rows), _cols(cols) {
  assert(r);
  assert(t);
  assert(0 < w);
  assert(0 < h);
...
std::shared_ptr<SpriteSheet> SpriteSheet::loadFromFile(const std::string &path,
    unsigned int rows, unsigned int cols, SDL_Renderer *rdr) {
  /* assertions */
  assert(rdr);
  /* tmp objects */
  ...
  try {
    ...
    if (nullptr == (surface = IMG_Load(path.c_str()))) {
      throw std::runtime_error(
          std::string(IMG_GetError()) + std::string(" : ") + path);
    }
    /* transform image to texture */
    if (nullptr == (texture = SDL_CreateTextureFromSurface(rdr, surface))) {
      throw std::runtime_error(
          std::string(SDL_GetError()) + std::string(" : ") + path);
    }
    SDL_FreeSurface(surface);
    surface = nullptr;
    /* create the sheet */
    auto sprite(
        std::make_shared<SpriteSheet>(rdr, std::move(texture), 1, 1, rows,
            cols));
...
void SpriteSheet::renderFrame(unsigned int row, unsigned int col,
    const SDL_Rect *dest) {
  assert(row < _rows);
  assert(col < _cols);
  SDL_Rect rect;
