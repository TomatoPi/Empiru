...
void ZoneGenerator::createZone(int size) {
  for (int i = 0; i < size; i++) {
    _vertexs.emplace_back((rand() % 10), (rand() % 10));
  }

  std::sort(_vertexs.begin(), _vertexs.end(),
      [&](const world::Position &a, const world::Position &b) -> bool {
        return a._x < b._x;
      });
...
void ZoneGenerator::addObject() {
  ...
  for (int i = 0; i < nb_object; i++) {
    ...
    world::Position position { xRand, yRand };
    _objects.push_back(position);
  }
}

float ZoneGenerator::calculateY(const world::Position &posA,
    const world::Position &posB, float xC) {
  float y, a, b, c;
